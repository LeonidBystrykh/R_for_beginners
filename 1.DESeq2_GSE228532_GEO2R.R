...
################################################################
#   Differential expression analysis with DESeq2 as generated online with GEO2R robot
library(DESeq2)
...
# sample selection and sample table
gsms <- "000111222"
...
#Major 2 lines of the DESeq2 package
ds <- DESeqDataSetFromMatrix(countData=tbl, colData=sample_info, design= ~Group)
ds <- DESeq(ds, test="LRT", reduced = ~ 1)  # Use LRT for all-around gene ranking

# extract results. Probably the 3rd most important line in the DESeq2 script 
r <- results (ds, alpha=0.05, pAdjustMethod ="fdr")
r
tT <- r[order(r$padj)[1:250],] #that's awful, it will be always 250 genes in the output if you do this way
tT <- merge(as.data.frame(tT), annot, by=0, sort=F) #add annotation. Yes we need gene names

tT <- subset(tT, select=c("GeneID","padj","pvalue","stat","baseMean","Symbol","Description")) #reduce columns

write.table(tT, file=stdout(), row.names=F, sep="\t") #too fast! you need to find tops of your list!

#figures do not plat active role in such script, because it is processed automatically, no human involved
...
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE228532 Frequencies of padj-values")
...
# volcano plot., actually regular plot from base R
...
# MD plot, the same story: it's a plot which used data from the results table
par(mar=c(4,4,2,1), cex.main=1.5)
...
legend("bottomleft", title=paste("Padj<", 0.05, sep=""), legend=c("down", "up"), pch=20,col=1:2)
abline(h=0)
palette(old.pal) # call predefine palette

# Venn diagram. It's not what you think
library(gplots)
all_res <- list()
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE228532", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
legend("topleft", groups, fill=palette(), bty="n")

# UMAP plot (multi-dimensional scaling). This is very wrong choice to my opinion. But well, it's just my opinion...
library(umap)
